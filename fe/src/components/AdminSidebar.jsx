...
const AdminSidebar = ({ user, handleLogout }) => {
    ...
    const menuItems = [
        ...
        { path: '/admin/flights', label: 'Quản lý chuyến bay', icon: '✈️' },
        { path: '/admin/airports', label: 'Quản lý sân bay', icon: '🏢' },
        { path: '/admin/ticket-classes', label: 'Quản lý hạng vé', icon: '🎫' },
        { path: '/admin/regulations', label: 'Quản lý quy định', icon: '⚖️' },
        { path: '/admin/revenue', label: 'Quản lý doanh thu', icon: '📊' },
    ];
        ...
        <div className="w-full md:w-[280px] flex-shrink-0">
            <div className="bg-white shadow-sm border border-gray-200 h-full flex flex-col">
                <div className="p-6 border-b border-gray-100 flex flex-col items-center gap-2">
                    ...
                    <span className="font-black text-xl text-[#333]">Admin Panel</span>
                </div>
                ...
                <nav className="flex flex-col text-[15px] text-[#333] flex-grow">
                    {menuItems.map((item) => {
                        ...
                        return (
                            ...
                            >
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-sm ${isActive ? 'bg-[#8dc63f]' : 'bg-gray-400'}`}>
                                    {item.icon}
                                </div>
                                {item.label}
                            </button>
                    ...
                    >
                        <div className="w-8 h-8 rounded-full flex items-center justify-center bg-blue-400 text-white text-sm">
                            🌐
                        </div>
                        Xem Trang Chủ
                    </button>
                ...
                <div className="p-4 border-t border-gray-100 bg-gray-50">
                    <div className="flex items-center gap-3">
                        <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#8dc63f] font-bold text-white shadow-sm">A</div>
